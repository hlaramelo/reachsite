...
import { MEDIA_CONTENT_EXTENDED } from "@/lib/constants";

const ITEMS_PER_PAGE = 6;

const TYPE_FILTERS = [
  { key: "todos", label: "Todos" },
  { key: "carta", label: "Cartas do Gestor" },
  { key: "video", label: "Vídeos & Podcasts" },
  { key: "artigo", label: "Artigos & Imprensa" },
];
...
export default function MidiaClient() {
  ...
  const sortedContent = useMemo(() => {
    return [...MEDIA_CONTENT_EXTENDED].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  }, []);

  const filteredContent = useMemo(() => {
    if (activeFilter === "todos") return sortedContent;
    return sortedContent.filter((m) => m.type === activeFilter);
  }, [activeFilter, sortedContent]);
            ...
            {TYPE_FILTERS.map((filter) => {
              const count =
                filter.key === "todos"
                  ...
                  : sortedContent.filter((m) => m.type === filter.key).length;
              return (
                <button
                  key={filter.key}
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedContent.map((item, i) => (
              <ScrollReveal key={`${item.title}-${item.date}`} delay={i * 80}>
                <a
                  href={item.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg hover:border-lime/30 transition-all duration-300 cursor-pointer h-full flex flex-col"
                >
                  {/* Video thumbnail */}
                  {item.type === "video" && (
                    <div className="aspect-video bg-navy-900/5 flex items-center justify-center relative">
                      <div className="w-14 h-14 rounded-full bg-lime/20 flex items-center justify-center group-hover:bg-lime/30 transition-colors">
                        <svg className="w-7 h-7 text-lime ml-0.5" fill="currentColor" viewBox="0 0 24 24">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-12">
              <button
                ...
                className="px-3 py-2 rounded-lg text-sm font-semibold text-navy-700 hover:bg-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              >
          ...
          <p className="text-center text-gray-400 text-sm mt-6">
            ...
            {Math.min(currentPage * ITEMS_PER_PAGE, filteredContent.length)} de{" "}
